...
import { MdArrowOutward  } from "react-icons/io";
  ...
  return (
    <div className='bg-gradient-to-br from-blue-50 via-white to-indigo-50 md:min-h-screen min-h-screen'>
      <div className="px-50 py-8 flex justify-between items-center">
        ...
        <div className="pt-15 ">
          <button className="flex items-center gap-2 text-gray-600 border border-gray-200 font-semibold hover:bg-gray-50 py-2 px-3 rounded-lg bg-white"><LuFilter className="font-semibold"/> Filters</button>
        </div>
      </div>
        ...
        {transactions.map((tx, index) => (
          <div key={tx.id || index}>
            ...
            <div className="flex justify-between items-center px-6 py-3">
              <div className="flex items-center gap-4">
                <div>
                  {tx.direction === 'up' ? <IoMdArrowRoundUp /> : <IoMdArrowRoundDown />}
                </div>
                <div>
              ...
              <div>
                <p
                  className={`flex items-center gap-1 font-bold text-md ${
                    tx.direction === 'up' ? 'text-green-600' : 'text-red-600'
                  }`}
                >
                  {tx.direction === 'up' ? <FiPlus /> : <FiMinus />} Rs {tx.amount}.00/-
                </p>
                <p className="text-[14px] text-gray-500 pt-2">Savings Account</p>
              </div>
            </div>
